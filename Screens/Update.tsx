...
const Update = ({navigation, route}: any) => {
  const item: IPassword = route.params.EditData;
  ...
  const [generatePass, setgeneratePass] = useState<string>(item.password);
  const [length, setlength] = useState<number>(item.password.length);
  ...
  const SavingPassword = async (data: any) => {
    try {
      ...
      if (state.Items.length > 0) {
        const found = state.Items.filter((it: IPassword) => it.id !== item.id);

        await AsyncStorage.setItem(
          'Passwords',
          JSON.stringify(encryptData([...found, data])),
        );
        dispatch({
          type: 'UPDATE_PASSWORD',
          payload: {Items: [...found, data]},
        });
        setTimeout(() => {
          setLoading(false);
  ...
  return (
    <Layout
      ...
      <View style={styles.AddPss}>
        <Text style={styles.addPssText}>Update Password</Text>
